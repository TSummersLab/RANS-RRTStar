...
Date created: Tue Oct 27 12:29:47 2020
(C) Venkatraman Renganathan, 2020.
...
class UKF:
    ...
    def prediction_step(self, sigmaPoints):
        ...
        for i in range(co):
            aprioriPoints[:, i] = self.new_motion_model(sigmaPoints[:, i])

            # Compute the mean and covariance of the transformed points
        aprioriOutput = self.compute_statistics(aprioriPoints, apriori_flag=1)
    ...
    def update_step(self, aprioriPoints):
        ...
        # Loop through and pass each and every sigmapoint
        for i in range(M):
            aposterioriPoints[:, i] = self.measurement_model(aprioriPoints[:, i])

        # Compute the mean and covariance of the transformed points    
        aposterioriOutput = self.compute_statistics(aposterioriPoints, apriori_flag=0)
    ...
    def new_motion_model(self, oldState):
        v = self.u_k[0]
        omega = self.u_k[1]
        x = oldState[0]
        y = oldState[1]
        theta = oldState[2]
        newState = oldState + [self.dT*v*np.cos(theta),
                               self.dT*v*np.sin(theta),
                               self.dT*omega]

        return newState

    def measurement_model(self, newState):  # TODO: DOUBLE CHECK
        """
    ...
    def compute_cross_covariance(self, funParam):
    ...
    def compute_statistics(self, inputPoints, apriori_flag):
    ...
    def compute_ukf_gain(self, funParam, aposterioriCovar):
        ...
        crossCovarMatrix = self.compute_cross_covariance(funParam)
    ...
    def estimate(self, ukf_params):
        ...
        aprioriOutput = self.prediction_step(sigmaPoints)
        ...
        # ukfOutput = {"x_hat": aprioriMean, "SigmaE": aprioriCovar}

        ###################### Aposteriori Update #############################
        # Compute the aposteriori output
        aposterioriOutput = self.update_step(aprioriPoints)
