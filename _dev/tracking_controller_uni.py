...
# This code supports a more advanced disturbance profile which is colored-in-time (time correlated),
# as well as being state-dependent e.g. more noise in the rolling direction than the transverse direction
...
def rollout(n, m, T, Ts, x0=None, w0=None, w_base_hist=None, u_hist=None,
    ...
    for t in range(T-1):
        ...
        if disturb:
            ...
            w = np.zeros(n)
        x_hist[t+1] = dtime_dynamics(x, u) + w
        u_hist[t] = u
        w_hist[t+1] = w
...
if __name__ == "__main__":
    ...
    for t in range(T):
        A_hist[t], B_hist[t] = dtime_jacobian(x_ref_hist[t], u_ref_hist[t])
